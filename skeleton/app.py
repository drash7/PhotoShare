...
def view_photos():
	...
	for i in range(len(pics)):
		...
		cursor.execute("SELECT email FROM Users WHERE user_id IN (SELECT user_id FROM Comments WHERE photo_id = '{0}')".format(pics[i][1]))
		emma = cursor.fetchall()
		com = list(zip(coma,emma))
		a = (pics[i])
		if len(coma) > 0:
			for x in coma:	
				a = (a + x)		
	...
	for i in range(len(pics3)):
		...
		if len(nolikes) > 0:
			...
			abc = (abc + bc)
		pics4.append(abc)
	return render_template('gallery.html', photos=pics, ex=pics4, h1 = coma, base64=base64)
...
def cl_photos():
	...
	if resp == 'yes':
		...
		conn.commit()
		return render_template('gallery.html', photos=pics, ex = pics4, h1 = coma, base64=base64, message='Like added!')
	elif email1 == email2:
		return render_template('gallery.html', photos=pics, ex = pics4, h1 = coma, base64=base64, message='You Cannot add a comment to your own photo')
	else:
		score = getUserScore(flask_login.current_user.id)
		cursor.execute("INSERT INTO Comments (ctext, photo_id, user_id) VALUES ('{0}', '{1}', '{2}')".format(comment, pic, email1))
		conn.commit()
		return render_template('gallery.html', photos=pics, ex = pics4, h1 = coma, base64=base64, message='Comment added!')
